...
function attachToCollusionPage(worker) {
   ...
   * that we attached to the web page running the Collusion UI. */
    ...
    var onRestored = function(connections){
        ...
        aggregate.emit("buildAggregate", connections);
        worker.port.emit('init', allConnections); // FIXME, should use filtered connections
    };

    aggregate.on("init", function(aggr){
        worker.port.emit('log', "= lib/ui.js aggregate on init =================");
        aggregateData = aggr;
        worker.port.emit('log', aggregateData.edges);
        worker.port.emit("aggregateInit", aggregateData);
    });
    ...
    var onAggregateUpdate = function(aggr){
        worker.port.emit('aggregateUpdated', aggr);
    };
    
    aggregate.on("updated", onAggregateUpdate);
