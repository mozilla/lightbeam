...
var whitelist = [];
// Blocklist - domains the user has blocked:
var blocklist = [];
...
function blockDomain(domain) {
  ...
  storage.blocklist = JSON.stringify(blocklist);
  ...
  var cookieEnumerator = cookieMgr.getCookiesFromHost(domain);
  ...
  console.log("Finished blocking domain " + domain);
...
function initCollusion() {
  ...
  // Load the blocklist we have stored from last time
  if (storage.blocklist) {
    blocklist = JSON.parse(storage.blocklist);
  }
  ...
  obSvc.add("cookie-changed", function(topic, data) {
    ...
    if (data == "added") {
      ...
      // see https://github.com/toolness/collusion/issues/86
    }
  });
  ...
  obSvc.add("http-on-modify-request", function(subject, topic, data) {
    ...
    if (blocklist.indexOf(domain) != -1) {
      ...
      thistab = window.gBrowser.mCurrrentBrowser;
      ...
      if (notification) {
         notification.label = message;
           ...
           notificationBox.appendNotification(message, "collusion-blocked",
