...
function queueInfo(info) {
  if (!(info.domain in log)) {
    log[info.domain] = { referrers: {},
                         ...
                         nocookie: true // Will be set to true if it sets a cookie
                         };
  }
...
obSvc.add("http-on-examine-response", function(subject, topic, data) {
  ...
  if (channel.referrer) {
    ...
    if (domain != referrerDomain) {
	  ...
	  // singe: Add all requests to third parties, not just those that set cookies
	  queueInfo({
		type: type,
		domain: domain,
      ...
      if (cookie) {
        ...
        if (cookieReallyExists) {
  		  log[domain].nocookie = false; // The site sets a cookie, so highlight it as a tracker or normal site
          //queueInfo({
            //type: type,
            //domain: domain,
            //referrer: referrerDomain
