...
var filter24hours = function(connection){
    ...
    if (Array.isArray(connection)){
        return connection[Connection.TIMESTAMP] > yesterday;
    }else{
...
function restore(){
    ...
    if ( storage.tempConnections ){
        ...
        console.error( storage.tempConnections)
...
//    console.error("=== addConnection isPrivate = " + connection._sourceTab.isPrivate);
    ...
    if (connectionFilter(connection)){
        ...
        Connection.emit('connection', conn);
    }
    if ( ! connection._sourceTab.isPrivate ){ // add it to the simple storage
        ...
        if ( captureAndStoreConnections ){
            storage.tempConnections.push(connection);
//            console.error("=====");
...
Connection.upload = function(){
    ...
    var connections = storage.connections.filter(function(connection){
        return ( connection.timestamp || connection[Connection.TIMESTAMP] ) > lastUpload;
