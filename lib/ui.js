...
const { Connection, getAllConnections } = require("./connection");
...
function attachToLightbeamPage(worker) {
  ...
  function onUIReady() {
    ...
    worker.port.emit("setPrefs", { contributeData: prefs.contributeData });
  }
  ...
  function onWorkerDetach() {
    ...
    this.port.removeListener("prefChanged", prefChanged);
    uiworker = null;
    this.destroy();
  }
