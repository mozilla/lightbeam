// All storage and upload logic in the addon process goes here.
"use strict";
...
function serializeConnections(connections) {
  let exportSet = {
    ...
    version: '1.1',
    uploadTime:  Date.now(),
    connections: excludePrivateConnections(connections)
  }
  return JSON.dumps(exportSet);
}
...
function checkStorageQuota(){
  while (ss.quotaUsage > 1){
    var sliceStart = Math.floor(ss.storage.connections.length * 0.5);
